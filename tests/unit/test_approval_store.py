...
def test_upsert_marks_pending_projection(tmp_path: Path) -> None:
    ...
    projection = store.get_projection("sess-123", "hash-pending")
...
def test_upsert_accepts_pending_projection(tmp_path: Path) -> None:
    db_path = tmp_path / "knowledge.db"
    store = ApprovalStore(db_path)
    store.init_schema()
    ...
    projection = store.get_projection("sess-456", "hash-pending")
